      ...
      <div class="right-ctn-title">
            <div class="left">
                ...
                <!-- <el-button size="mini" type="primary" @click="ruleSearch">查询</el-button> -->
                 <el-button  type="success" icon="el-icon-folder-add" style="margin-left: 10px;" @click="treatSave">服务目录对应保存</el-button>
                <el-table
...
export default {
  ...
  methods: {
   ...
   /*  foulPinyin(val) {
    ...
    leftClick(row) {
      console.log(row);
      this.treatCode = row.AKA100;
      this.treatName = row.AKA102;
      this.$message("已选中");
    },
    // his服务目录初始化列表
    getRulelist() {
      axios({
        method: "post",
        url: axios.PARK_API + "/medical_reimbursement/his/querHisServiceList",
        data: {},
        headers: {
          "Content-Type": "application/json;charset=UTF-8"
        }
    ...
    rightClick(row) {
      console.log(row);
      this.hisCode = row.AKA100;
      this.hisName = row.AKA102;
      this.sinmoney = row.AKA068;
      this.$message("已选中");
    },
    ...
    treatSave() {
      axios({
        ...
        .then(res => {
          if (res.data.code == 0) {
            this.$message.success("服务目录对应保存成功");
          } else {
            this.$message.error(res.data.msg);
          }
        })
